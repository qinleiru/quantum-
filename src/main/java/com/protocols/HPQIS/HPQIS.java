...
public class HPQIS {
    ...
    public static DoubleState run(TextComponent textArea, String agents, String secret) {
        ...
        if ("00".equals(secret)) {
            ...
            Alice = new Sender(highAgents, lowAgents, doubleState);
            textArea.setCommText(df.format(System.currentTimeMillis()) + " " + "传送的秘密量子比特为"+QuantumTools.showBinaryState(doubleState));
        } else if ("01".equals(secret)) {
            DoubleState doubleState=new DoubleState(new double[]{0,1,0,0});
            Alice = new Sender(highAgents, lowAgents, doubleState);
            textArea.setCommText(df.format(System.currentTimeMillis()) + " " + "传送的秘密量子比特为"+QuantumTools.showBinaryState(doubleState));
        } else if ("10".equals(secret)){
            DoubleState doubleState=new DoubleState(new double[]{0,0,1,0});
            Alice = new Sender(highAgents, lowAgents,doubleState);
            textArea.setCommText(df.format(System.currentTimeMillis()) + " " + "传送的秘密量子比特为"+QuantumTools.showBinaryState(doubleState));
        }else if ("11".equals(secret)){
